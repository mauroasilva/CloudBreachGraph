...
3. **Observed connections** (up to `FLOW_LOG_MAX_LOOKBACK_DAYS = 60` days of records). `describe-flow-logs`
   ...
   - **S3** (`s3`): `s3api list-objects-v2` under the destination ARN's bucket/prefix (filtered to
     ...
     for 30 days (they're immutable), so re-runs don't re-download. Each source is read **largest
     object first** (`_size_descending_keys`, using the `Size` that `list-objects-v2` already returns
     — no extra call): an all-NODATA object compresses tiny, so descending size front-loads the
     data-bearing objects and pushes NODATA to the tail. Reading then stops early on a run of NODATA
     (consecutive empty parses), with two thresholds: a **cold** source that has produced no records
     is abandoned after `_FLOW_LOG_PROBE_OBJECTS` (25) consecutive empties (all-NODATA/unrecognised —
     a conservative sample, since there's no positive evidence yet); a source that has **already**
     yielded records has its smaller NODATA **tail trimmed** after just `_FLOW_LOG_TAIL_STREAK` (3)
     consecutive empties (strong evidence once the source is known-real). So a 15k-data + 5k-NODATA
     source reads the 15k data and skips the dead tail instead of downloading it. A single all-NODATA
     VPC never aborts a run whose other VPCs have data; the run raises `FlowLogFetchError` only if
     **every** source yields zero records (preserving the loud single-VPC signal). Download order
     never affects output — records are sorted downstream.
...
- **Converting existing output → HTML** (`cloudbreachgraph-to-html`, `convert.py`): an
  ...
  *secondary* objective (crossings ~halve on the example graph, 39→18) that never sacrifices the
  ...
  VPC by the edge's **semantics** (`_edge_vpcs`), not by a node's single home group: a reachability
  edge (`can_reach*`) lands in the VPC of the resource it reaches (its target); a flow `connects_to`
  lands in the VPC of its ENI end when the other end is a `flow_peer`, or — for an *observed flow
  between two real ENIs* — in **both** their VPCs; and a structural edge lands in the one VPC both
  endpoints share. Both of an edge's endpoints are then pulled into every VPC it lands in, so a
  source that relates to several VPCs — a CIDR, an `internet`/`flow_peer`, **or a security group
  referenced across a peering** — appears in every VPC it reaches, and a cross-VPC ENI↔ENI flow
  shows up in both endpoints' files (each with the other ENI pulled in). Unassigned nodes and edges
  that resolve to no VPC are still dropped. This is on purpose different from `_vpc_group_of`, which
  single-assigns a shared source (correct for the overlap-free single-page layouts, where each node
  is drawn once, but exposure-hiding when splitting). It reuses `write_layout_html` per sub-graph,
  so the layout flags (`--ringed`/`--hierarchical`/`--optimize-passes`/`--no-security-groups`) and
  the size guard / per-file `.dot` fallback all apply to every VPC file.
